...
Neterminál \code{element\_body} bol iba pomocný neterminál z ľavej faktorizácie. V tejto časti ho už neponecháme ako samostatný problémový bod, ale nahradíme ho jednoznačným rozhodnutím podľa prvého symbolu obsahu elementu.

Pred úpravou sme mali:
...
Konflikt vznikol preto, že prázdny obsah sa dal odvodiť dvoma rôznymi cestami:

\begin{lstlisting}
element_body -> words endtag
words -> epsilon
\end{lstlisting}
...
\begin{lstlisting}
element_body -> elements endtag
elements -> epsilon
\end{lstlisting}

Preto pri vstupe \code{</} nebolo jasné, či má tabuľkový analyzátor použiť vetvu cez \code{words}, alebo vetvu cez \code{elements}. Prázdny obsah však v jazyku potrebujeme iba raz. Preto obsah elementu rozdelíme priamo na tri prípady:
...
\begin{itemize}
    \item ak obsah začína znakom z množiny \(W\), použije sa pravidlo \code{element\_content -> word},
    \item ak obsah začína terminálom \code{<}, použije sa pravidlo \code{element\_content -> element},
    \item ak obsah začína terminálom \code{</}, použije sa pravidlo \code{element\_content -> epsilon}.
\end{itemize}

Pre neterminál \code{element\_content} potom dostaneme:

\[
\First(\texttt{word}) = W,
\qquad
\First(\texttt{element}) = \{\texttt{<}\},
\qquad
\Follow(\texttt{element\_content}) = \{\texttt{</}\}.
\]

FIRST/FIRST konflikt zmizne, pretože:

\[
W \cap \{\texttt{<}\} = \emptyset.
\]

FIRST/FOLLOW konflikt pri epsilon pravidle tiež nevznikne, pretože epsilon pravidlo sa zapisuje podľa \(\Follow(\code{element\_content})\), teda iba pod \code{</}:

\[
\bigl(\First(\texttt{word}) \cup \First(\texttt{element})\bigr)
\cap
\Follow(\texttt{element\_content})
=
(W \cup \{\texttt{<}\}) \cap \{\texttt{</}\}
=
\emptyset.
\]

Tým sa odstráni aj pôvodný konflikt pri \code{word\_tail}. V medzivýslednej gramatike sa po jednom \code{word} mohlo podľa pravidla \code{words -> word words} začať ďalšie \code{word}, preto \(\Follow(\code{word\_tail})\) obsahovalo množinu \(W\). Po odstránení \code{words} sa \code{word} používa priamo v pravidle \code{element\_content -> word}; za ním nasleduje už iba \code{endtag}. Preto platí:

\[
\Follow(\texttt{word\_tail}) = \{\texttt{</}\}
\]

a teda:

\[
\First(\texttt{word}) \cap \Follow(\texttt{word\_tail})
=
W \cap \{\texttt{</}\}
=
\emptyset.
...
Výsledná úprava teda nie je nový významový krok navyše, ale zlúčenie prázdneho obsahu do jednej vetvy a odstránenie nejednoznačného delenia textu na viacero \code{word} pravidiel.
